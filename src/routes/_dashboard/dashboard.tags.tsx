    ...
    <div className="flex flex-1 flex-col gap-6 bg-ns-bg">
      ...
      <section className="relative overflow-hidden rounded-xl border border-ns-border bg-ns-panel p-6 shadow-lg">
        ...
        <div className="relative">
          ...
          <h1 className="mb-2 flex items-center gap-2 text-xl font-bold text-ns-text">
            ...
            <Tag size={16} className="text-ns-primary-lt" />
          </h1>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {tagsData.map((tag) => (
          <button
            key={tag.name}
            className="group animate-fade-in flex cursor-pointer items-center justify-between rounded-xl border border-ns-border bg-ns-panel p-3 text-left transition-all hover:border-ns-border-md hover:bg-ns-hover/30"
          >
            ...
            <span className="rounded border border-ns-border/40 bg-ns-input px-2 py-0.5 text-[0.58rem] font-bold text-ns-muted">
              {tag.count}
            </span>
          </button>
        ))}
